// src/components/Contribute/Skill/YamlFileUpload.tsx
...
interface SkillYamlFileUploadProps {
  onUploadSuccess: (data: SkillYamlData) => void;
  onUploadError: (title: string, message: string) => void;
}

const SkillYamlFileUpload: React.FC<SkillYamlFileUploadProps> = ({ onUploadSuccess, onUploadError }) => {
  ...
  const handleFileInputChange = (event: DropEvent, file: File) => {
    ...
    reader.onload = () => {
      ...
      try {
        const yamlData = yaml.load(fileContent) as SkillYamlData;
        if (yamlData) {
...
export default SkillYamlFileUpload;
